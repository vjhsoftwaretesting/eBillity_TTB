...
public class C04_clientPage_AllFilter extends Client_BaseClass {
	
	
	@Test(dataProvider="driveTest")
	public void all_TableValidate (String per_Org, String fName, String lName, String orgName) throws InterruptedException {
		...
		Thread.sleep(2000);
		driver.findElement(By.xpath("//*[@class='link-container']/div[contains(text(),'All')]")).click();
		Thread.sleep(2000);
		...
		for(WebElement name : tableClientElements) {
			...
			if(clientName.equals(expectedName)) {
				...
				System.out.println("Created client is present in 'All' filter in table at page "+ nextPage);
				break;
				}
		}
		...
		Assert.assertEquals(expectedName, actualName);
			...
			for(WebElement names : tableClientElementss) {
				...
				if(clientNames.equals(expectedName)) {
					...
					System.out.println("Created client is present in 'All' filter in table");
					break;
					}
			}
			Assert.assertEquals(expectedName, actualName);
